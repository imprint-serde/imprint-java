...
public class ImprintRecord {
    ...
    /**
     * Used by {@link ImprintRecordBuilder} with sorted field data.
     * Creates directory buffer from field data and calculated offsets.
     *
     * @param sortedFields Array of FieldData objects sorted by ID
     * @param offsets Array of payload offsets corresponding to each field
     * @param fieldCount Number of valid fields to process
     */
    static ByteBuffer createDirectoryBufferFromSorted(Object[] sortedFields, int[] offsets, int fieldCount) {
        if (fieldCount == 0)
            return createEmptyDirectoryBuffer();

        int size = calculateDirectorySize(fieldCount);
        var buffer = ByteBuffer.allocate(size);
        buffer.order(ByteOrder.LITTLE_ENDIAN);
        VarInt.encode(fieldCount, buffer);

        //this ends up being kind of a hotspot for some reason, probably boundary checking.
        //Direct writes might help a bit it could get difficult since pretty much all the other
        //frameworks just go straight for Unsafe
        for (int i = 0; i < fieldCount; i++) {
            var fieldData = (ImprintRecordBuilder.FieldData) sortedFields[i];
            buffer.putShort(fieldData.id);
            buffer.put(fieldData.value.getTypeCode().getCode());
    ...
    private static ByteBuffer createEmptyDirectoryBuffer() {
