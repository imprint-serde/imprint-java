...
public final class ImprintBuffers {
    ...
    private TreeMap<Integer, Directory> parsedDirectory;
     ...
     */
    public ImprintBuffers(List<? extends Directory> sortedDirectory, ByteBuffer payload) {
    ...
    public ImprintBuffers(Map<Integer, ? extends Directory> parsedDirectory, ByteBuffer payload) {
        this.directoryBuffer = ImprintBuffers.createDirectoryBufferFromSortedMap(Objects.requireNonNull(parsedDirectory));
        this.payload = payload.asReadOnlyBuffer();
        this.parsedDirectory = (TreeMap<Integer, Directory>) parsedDirectory;
     ...
     */
    public ByteBuffer getFieldBuffer(Directory entry) throws ImprintException {
        if (entry == null)
            return null;
     ...
     */
    public Directory findDirectoryEntry(int fieldId) throws ImprintException {
        if (directoryParsed)
     ...
     */
    private Directory findFieldEntryInRawDirectory(int fieldId) throws ImprintException {
     ...
     */
    private int findEndOffset(Directory entry) throws ImprintException {
        if (directoryParsed) {
    ...
    private int findNextOffsetInParsedDirectory(int currentFieldId) {
        ...
        if (nextEntry != null)
            return nextEntry.getValue().getOffset();
        else
    ...
    private void ensureDirectoryParsed() {
        ...
        try {
            ...
            for (int i = 0; i < count; i++) {
                ...
                this.parsedDirectory.put((int) entry.getId() , entry);
            }
     ...
     */
    static ByteBuffer createDirectoryBuffer(Collection<? extends Directory> directory) {
        ...
        ArrayList<? extends Directory> sortedDirectory;
        if (directory instanceof ArrayList && isSorted((ArrayList<? extends Directory>)directory)) {
            sortedDirectory = (ArrayList<? extends Directory>) directory;
        } else {
    ...
    static ByteBuffer createDirectoryBufferFromSortedMap(Map<Integer, ? extends Directory> directoryMap) {
    ...
    private static boolean isSorted(ArrayList<? extends Directory> list) {
     ...
     */
    private static void serializeDirectoryEntry(Directory entry, ByteBuffer buffer) {
